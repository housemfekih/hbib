...
class templateChampController extends Controller
{
    ...
    public function newTmpAction(Request $request)
    {
		...
		//	$url = 'http://localhost/resume_builder/web/app_dev.php/templatechamp/testTmp/'.$templatehtmlFile->getFichierTemplate().'/'.$array_string;
         
        
$url = 'http://127.0.0.1:8000/app.php/templatechamp/testTmp/'.$templatehtmlFile->getFichierTemplate().'/'.$array_string;
            
            // $this->get('knp_snappy.image')->generate($url,$pdf);
          ...
          # $res = ["dichCv" => $pdf , "contCv" => file_get_contents($url)];
          $html = $this->renderView('templates/2c91cd0dfea03046802635d383be6776.html.twig', array(
        ...
        //     'image.jpg'
        // );
        $image=$this->get('knp_snappy.image')->getOutputFromHtml($html);
        dump('aa',$image);
			$cccv= file_get_contents($url);
            $res = ["dichCv" => $pdf , "contCv" => $cccv];
		...
		$arrays = explode(";",$array_string);
			...
			foreach ($arrays as $array_st) {
				...
				if (sizeof($arrays_st) > 2) {
					...
					if (!array_key_exists("ch".$arrays_st[0],$arr)) {
						$arr["ch".$arrays_st[0]] = [["title" => $index, "content" => $arrays_st[2]]];
					}else {
						array_push($arr["ch".$arrays_st[0]],["title" => $index, "content" => $arrays_st[2]]);
					}
